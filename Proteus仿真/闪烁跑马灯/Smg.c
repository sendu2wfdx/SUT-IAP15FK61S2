...
void Pao(unsigned int i)
{
	...
	P0=Dig[s1];	Wei(1);	delayms(100);
	keyscan();
	P0=Dig[s2];	Wei(2);	delayms(100);
	keyscan();
	P0=Dig[s3];	Wei(3);	delayms(100);
	keyscan();
	P0=Dig[s4];	Wei(4);	delayms(100);
	keyscan();
}
...
void Alert(void)
{
	while(!flag)
	{
		P0=Dig[15];	Wei(1);	delayms(2);
		P0=Dig[15];	Wei(2);	delayms(2);
		P0=Dig[15];	Wei(3);	delayms(2);
		P0=Dig[15];	Wei(4);	delayms(2);
		keyscan();
	}
}